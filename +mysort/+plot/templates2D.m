function templates2D(T, electrodePositions, maxChan, absThresh, varargin)
    P.fh = [];
    P.ah = [];
    P.stacked = 1;
    P.plotLegend = 1;
    ...
    for i_=1:size(T,3)
        ...
        if P.stacked==1
            mysort.plot.waveforms2D(wfs, electrodePositions, 'AxesHandle', P.ah, 'plotArgs', ...
                {marker, 'color', col, 'linewidth', 2},...
                'absThreshold', absThresh, 'maxNumberOfChannels', maxChan);
        else
            mysort.plot.waveforms2D(wfs, electrodePositions, 'AxesHandle', P.ah(i_), 'plotArgs', ...
