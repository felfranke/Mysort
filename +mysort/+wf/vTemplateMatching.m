    ...
    for taui = 1:length(tauRange)
        ...
        for k = 1:nT
            if ndims(FupShiftDown)==2
                [D_(:,k) subTauIdx(:,k)] = max(X(:,idx_tau)*squeeze(FupShiftDown(k,:)'), [], 2);
            else
                [D_(:,k) subTauIdx(:,k)] = max(X(:,idx_tau)*squeeze(FupShiftDown(k,:,:)), [], 2);
            end
            [D(:,k) choice] = max([D(:,k) D_(:,k)], [], 2);
            % choice is 1 if the old value was greater and two if we found
            % a new maximum of the matching. In the later case, update the
            % shift
            maxTaus(choice==2,k) = tau - subTauRange(subTauIdx(choice==2,k));
            tauIdx(choice==2,k) = taui;
        end
    end